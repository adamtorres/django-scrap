...
class IngestCommand(BaseCommand):
    ...
    def text_to_date(self, raw_row_data, fields, default=None, date_format="%Y-%m-%d"):
        for field in fields:
            value = raw_row_data[self.field_index[field]]
            if value:
                try:
                    raw_row_data[self.field_index[field]] = parser.parse(value).strftime(date_format)
                except parser.ParserError:
                    raw_row_data[self.field_index[field]] = default
            else:
                raw_row_data[self.field_index[field]] = default
        return raw_row_data

    def text_to_number(self, raw_row_data, fields, _type, default):
        """
